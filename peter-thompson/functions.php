...
// Kill WPML's own hreflang tags on property pages and print clean ones
...
function ptre_fix_property_hreflang_tags() {
  ...
  // 1) Stop WPML from printing its own <link rel="alternate"> tags
  remove_action( 'wp_head', [ 'WPML_SEO_HeadLangs', 'head_langs' ] );

  // 2) Our base URL for this property
  $base_url = get_permalink();

  // 3) Only output the two real languages: EN → en-CA, FR → fr-CA
  $langs = [
    'en' => 'en-CA',
    'fr' => 'fr-CA',
  ];
  foreach ( $langs as $code => $hreflang ) {
    // WPML gives us the correct permalink in that language
  ...
  // 4) Finally, print exactly one x-default pointing at our English (Canada) URL
  $default_url = apply_filters( 'wpml_permalink', $base_url, 'en' );
  printf(
